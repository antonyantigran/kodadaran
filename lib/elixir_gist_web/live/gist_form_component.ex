      ...
      <.form for={@form} phx-submit="create" phx-change="validate" phx-target={@myself}>
        <div class="justify-center px-28 w-full space-y-4 mb-10">
          ...
          <.input
            ...
            placeholder="Կոդի նկարագրություն..."
            autocomplete="off"
            phx-debounce="blur"
          />
          <div>
            <div class="flex p-2 items-center bg-emDark rounded-t-md border">
              <div class="w-[300px] mb-2">
                <.input
                  field={@form[:name]}
                  placeholder="Ֆայլի անուն և ընդլայնում․․․"
                  autocomplete="off"
                  phx-debounce="blur"
                />
              </div>
          ...
          <div class="flex justify-end">
            <%= if @id == :new do %>
              <.button class="create_button" phx-disable-with="Creating...">Ստեղծել</.button>
            <% else %>
              <.button class="create_button" phx-disable-with="Updating...">Թարմացնել</.button>
            <% end %>
          </div>
        </div>
      </.form>
